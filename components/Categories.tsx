...
export default function Categories() {
  return (
    <section id="categories" className="py-10 text-center bg-green-800">
      ...
      <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto px-6 ">
        {categories.map((cat, i) => (
          <div
            key={i}
            className="border-2 bg-green-100 transparent border-gray-300 p-6 rounded-xl hover:shadow-lg hover:border-black hover:bg-white transition duration-200 cursor-pointer"
          >
            <span className="font-semibold">{cat}</span>
            <span className="block mt-2 text-sm text-gray-500">
              {subcategories[cat].map((s, i) => (
